...
class FindActivity : AppCompatActivity() {
    ...
    private fun setRecyclerView(pathItem: ArrayList<FindData>, path: String) {
        addPathList.clear()
        findAdapter = FindRecyclerAdapter(pathItem, this@FindActivity, path)
        ...
        findAdapter.itemClick = object : FindRecyclerAdapter.ItemClick {
            override fun onItemClick(view: View?, position: Int) {
                if(!findAdapter.items[position].isEPUB){
                    var paths = findAdapter.items[position].path!!.split("/")
                    var nowpath = paths.last()
                    text_path.text = nowpath
                    setRecyclerView(getDirectoryList(findAdapter.items[position].path!!), findAdapter.items[position].path!!)
                }
            }
        }
    }
...
//        Log.d("dudco", );
    ...
    private fun checkPermission(path: String) {
        if ( checkSelfPermission(Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            ...
            text_path.text = path
            setRecyclerView(getDirectoryList(path), path)
        }
    }
